...
const controlador = require('../planentrenamiento/controlador');

const router = express.Router();

router.get('/', todos);//trae TODOS LOS DATOS
router.get('/:ID_PlanEntrenamiento', uno);//trae UN SOLO DATO, lo trae con la cedula
...
async function uno (req,res,next) {
    try {
        const items = await controlador.uno(req.params.ID_PlanEntrenamiento);
        respuesta.success(req, res, items, 200);
    } catch (error) {
        //respuesta.error(req,res,err,500);
        next(err);
